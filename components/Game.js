...
class ElementStyleProvider {
  constructor(size) {
    ...
    const rotated_triangle_shape_path = regular_polygon_path([size / 2, 3 * size / 8], 11 * size / 16,
      3, Math.PI / 2);
    ...
    const rotated_square_shape_path = regular_polygon_path([size / 2, size / 2], 8 * size / 14,
      4, 0);
    const rounded_rotated_square_shape_path = regular_polygon_path([size / 2, size / 2], 8 * size / 14,
      4, 0, Math.floor(0.1 * size));

    const pentagon_shape_path = regular_polygon_path([size / 2, size / 2], 9 * size / 16,
      5, -Math.PI / 2);
    ...
    this.shape_paths = [
      triangle_shape_path, square_shape_path, pentagon_shape_path,
